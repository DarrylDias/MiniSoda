...
You can deploy your MiniSoda website via git, rsync or Amazon S3. The deployment system ships with the [octopress-deploy][] gem which extends the Octopress CLI with the `deploy` command.
...
you never commit sensitive information to your repository. **his will generate a _deploy.yml file in your current directory which you can edit to add any necessary configuration. Remember to add your configuration to .gitignore to be sure you never commit sensitive information to your repository. Here is an example of git deployment configuration

## Git Deployment Configuration

Only `git_url` and `site_dir` is important to be edited.
...
Simply make sure your configuration points to the root directory of your static site. Then tell Octopress to deploy it.

```
octopress deploy
```

You can see the live demo [here](https://darryldias.github.io/).